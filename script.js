  ...
  function game(player1Name, player2Name) {
    ...
    function runGameInConsole() {
      ...
      while (true) {
        ...
        let turnResult = gameBoardObj.placeXorO(
        ...
        if (turnResult === -2) {
          ...
          console.log(
            `${playerArray[currPlayerInd].getName()} has won the game!!!!`
          );
          const result = {
            player1: player1,
            player2: player2,
...
function game(player1Name, player2Name) {
  ...
  function getPlayerInput(playerObj) {
    ...
    while (true) {
      row = prompt(
        `${playerObj.getName()}, please select the row number (1, 2, or 3) representing the cell for which you'd like to place an "${playerObj.getSymbol()}", then hit ENTER: `
      );
      ...
      col = prompt(
        `${playerObj.getName()}, please select the column number (1, 2, or 3) representing the cell for which you'd like to place an "${playerObj.getSymbol()}", then hit ENTER: `
      );
